...
import { adminGetCoaches, adminCreateCoach, adminUpdateCoach, adminDeleteCoach } from '@/actions/admin'
...
export function AdminCoaches({ onCoachesChange }: { onCoachesChange?: (coaches: Coach[]) => void }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    const payload = { ...draft, slug: draft.slug || slugify(draft.displayName) }
    if (editing) {
      await adminUpdateCoach(editing.id, payload)
    } else {
      await adminCreateCoach(payload)
    }
    setSaving(false)
    setCreating(false)
    setEditing(null)
    ...
    <div className="flex h-full flex-col">
      <div className="flex h-[60px] shrink-0 items-center justify-between px-5 pt-4">
        <div>
          ...
          <div className="text-[11px] text-muted-foreground">{rows.length} coach{rows.length === 1 ? '' : 'es'}</div>
        </div>
      </div>
        ...
        <SheetContent side="right" className="flex w-full flex-col gap-0 overflow-hidden sm:max-w-xl">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-6">
            <div className="space-y-1.5">
              ...
              <input value={draft.displayName} onChange={(e) => setDraft({ ...draft, displayName: e.target.value })} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <input value={draft.location} onChange={(e) => setDraft({ ...draft, location: e.target.value })} placeholder="City" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring" />
              </div>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="flex items-center justify-between rounded-lg border border-border/60 px-3 py-2.5">
              ...
              <button
                ...
                className={cn('relative h-5 w-9 rounded-full transition-colors', draft.active ? 'bg-foreground' : 'bg-border')}
              >
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium">Certifications <span className="font-normal text-muted-foreground">(one per line)</span></label>
              <textarea rows={3} value={draft.certifications.join('\n')} onChange={(e) => setDraft({ ...draft, certifications: e.target.value.split('\n').map((s) => s.trim()).filter(Boolean) })} className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" />
            </div>
            <div className="space-y-1.5">
              <label className="text-sm font-medium">Greeting message</label>
              <p className="text-[11px] text-muted-foreground">Shown as the first message in the member's Message Center.</p>
              <textarea rows={4} value={draft.systemPrompt} onChange={(e) => setDraft({ ...draft, systemPrompt: e.target.value })} placeholder={`Hi — ${draft.displayName.split(' ')[0] || 'Coach'} here, your advisor. How can I help you today?`} className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" />
            </div>
          </div>
          <div className="flex shrink-0 items-center justify-end gap-2 border-t border-border/60 bg-background px-6 py-3">
            ...
            <button onClick={save} disabled={saving || !draft.displayName.trim()} className="rounded-md bg-foreground px-4 py-2 text-sm font-medium text-background hover:opacity-90 disabled:opacity-50">
